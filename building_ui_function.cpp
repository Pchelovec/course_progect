...
void MainWindow::load_info_level_building_GrBox()
{
    ...
    ui->new_standart_need_special_worker_FCB->view()->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOn);
    ui->new_standart_need_special_worker_FCB->view()->setCornerWidget(new QSizeGrip(ui->new_standart_need_special_worker_FCB));
...
void MainWindow::set_dop_price()
{
    ...
    dir t;
    ui->pore_pay_procent_SpB->setValue(t.read_procent_File());//загрузка из файла
}
...
{
    ui->new_standart_material_name->clear();
    ui->new_standart_material_name->view()->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOn);
    ui->new_standart_material_name->view()->setCornerWidget(new QSizeGrip(ui->new_standart_material_name));
    material temp;
    temp.name=val;
    QList <material> list=QUERY->mater_like_my(temp);
    int row=list.size();
...
{
    ui->new_standart_ned_materisl_naznach->clear();
    ui->new_standart_ned_materisl_naznach->view()->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOn);
    ui->new_standart_ned_materisl_naznach->view()->setCornerWidget(new QSizeGrip(ui->new_standart_ned_materisl_naznach));
    material temp;
    temp.naznach=val;
    QList <material> list=QUERY->mater_like_my(temp);
