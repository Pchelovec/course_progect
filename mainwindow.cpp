...
{
    split_eqw();
    save_new_eqw();//занести в БД скрыть 2 часть, очистить
}
...
{
    split_worker();
    save_worker();//занести в БД скрыть 2 часть, очистить
}
...
void MainWindow::on_del_from_brig_clicked()
{
    if (ui->eqw_brigada_RB->isChecked())
    {      
        dell_eqw_from_brig();//удалить оборудование от бригады
    }
    else
    {
        if (ui->worker_brigada_RB->isChecked())
